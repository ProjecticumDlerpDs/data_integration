...
expression_matrix <- ReadMtx(
  mtx = file.path("~/data_integration/bewerkte_data/e75_count_matrix.mtx.gz"),
  features = file.path(datadir, "e75_feature_metadata.csv.gz"),
  cells = file.path(datadir, "e75_sample_metadata.csv"), 
  feature.column = 1,
  feature.sep = ",",
  skip.feature = 1,
  cell.sep = ",",  
...
e75 <- CreateSeuratObject(counts = expression_matrix,
...
```{r QC and selecteren van cellen figuur 1, fig.cap=Vioolplot: Er zijn drie vioolplots naast elkaar weergegeven. In alle drie de vioolplots wordt op de x-as de dataset E7.5 weergegeven. Elk stipje is een cel. Links wordt op de y-as het aantal unieke features weergegeven. In het midden staat op de y-as het aantal RNA binnen de cel. Rechts wordt op de y-as het percentage mitochondriale RNA weergegeven.}
vlnfeature <- VlnPlot(e75, 
...
e75 <- subset(e75, subset = nFeature_RNA > 200 & nFeature_RNA < 6000 & percent.mt < 6)
```
...
e75 <- FindVariableFeatures(e75, selection.method = "vst", nfeatures = 2000)

top10 <- head(VariableFeatures(e75), 10)

plot1 <- VariableFeaturePlot(e75)
plot2 <- LabelPoints(plot = plot1, points = top10, repel = TRUE)
```

```{r Kenmerkselectie figuur 3, fig.cap="VariableFeaturePlot: Op de x-as wordt de gemiddelde expressie weergegeven. Op de y-as wordt de standaard variantie weergegeven. De 2000 cellen met de meest variërende kenmerken zijn rood gekleurd en de 10 cellen met de meest variërende genen zijn aangegeven met hun naam."}
plot2
```
...
print(e75[["pca"]], dims = 1:5, nfeatures = 5)
```
...
VizDimLoadings(e75, dims = 1, reduction = "pca")
```
...
DimPlot(e75, reduction = "pca") + NoLegend()
```

```{r Lineaire dimensionale reductie uitvoeren figuur 6, fig.cap="DimHeatmap, PC1 wordt weergegeven met als de verschillende genen in de rijen, en verschillende cellen in de kolommen. Paars is een lage PCA-score, geel is een hoge PCA-score. Clusters worden hiermee gevisualiseerd"}
DimHeatmap(e75, dims = 1, cells = 454, balanced = TRUE)
```
...
e75 <- FindNeighbors(e75, dims = 1:8)
...
cluster2.markers <- FindMarkers(e75, ident.1 = 2)
head(cluster2.markers, n = 5)
cluster5.markers <- FindMarkers(e75, ident.1 = 0, ident.2 = c(1))
head(cluster5.markers, n = 5)
e75.markers <- FindAllMarkers(e75, only.pos = TRUE)
e75.markers %>%
    group_by(cluster) %>%
    dplyr::filter(avg_log2FC > 1)
cluster0.markers <- FindMarkers(e75, ident.1 = 0, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)
```

table(Idents(e75_seuratobject))

```{r Clusterbiomarkers figuur 9, fig.cap="ViolinPlot: Het expressielevel van het gen MS4A1 wordt per cluster weergegeven. In cluster 3 komt dit gen het meest tot expressie."}
VlnPlot(e75, features = c("Tenm3-intron"))
```

```{r Clusterbiomarkers figuur 10, fig.cap="FeaturePlot:Op de x-as staat umap_1 en op de y-as staat umap_2. De expressie van MS4A1 wordt aangegeven in het paars."}
FeaturePlot(e75, features = c("Tenm3-intron"))
e75.markers %>%
    group_by(cluster) %>%
    dplyr::filter(avg_log2FC > 1) %>%
    slice_head(n = 5) %>%
...
DoHeatmap(e75, features = top5$gene) + NoLegend()
